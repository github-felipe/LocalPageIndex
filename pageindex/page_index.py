...
################### check title in page #########################################################
async def check_title_appearance(item, page_list, is_local, start_index=1, model=None):
    title=item['title']
...
async def check_title_appearance_in_start_concurrent(structure, page_list, is_local, model=None, logger=None):
    if logger:
    ...
    for item in structure:
        if item.get('physical_index') is not None:
            ...
            tasks.append(check_title_appearance_in_start(item['title'], page_text, is_local=is_local, model=model, logger=logger))
...
def extract_toc_content(content, is_local, model=None):
    prompt = f"""
    ...
    if_complete = check_if_toc_transformation_is_complete(content, response, is_local=is_local, model=model)
    ...
    new_response, finish_reason = ChatGPT_API_with_finish_reason(model=model, prompt=prompt, chat_history=chat_history, is_local=is_local)
...
def toc_extractor(page_list, toc_page_list, model, is_local):
    ...
    has_page_index = detect_page_index(toc_content, is_local=is_local, model=model)
...
def toc_transformer(toc_content, is_local, model=None):
    ...
    last_complete, finish_reason = ChatGPT_API_with_finish_reason(model=model, prompt=prompt, is_local=is_local)
    if_complete = check_if_toc_transformation_is_complete(toc_content, last_complete, is_local=is_local, model=model)
    if if_complete == "yes" and finish_reason == "finished":
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        new_complete, finish_reason = ChatGPT_API_with_finish_reason(model=model, prompt=prompt, is_local=is_local)
...
def find_toc_pages(start_page_index, page_list, opt, is_local, logger=None):
    ...
    while i < len(page_list):
        ...
        if i >= opt.toc_check_page_num and not last_page_is_yes:
            break
        detected_result = toc_detector_single_page(page_list[i][0], is_local=is_local, model=opt.model)
...
def process_no_toc(page_list, is_local, start_index=1, model=None, logger=None):
    page_contents=[]
    token_lengths=[]
    ...
    toc_with_page_number= generate_toc_init(group_texts[0], is_local=is_local, model=model)
    for group_text in group_texts[1:]:
        toc_with_page_number_additional = generate_toc_continue(toc_with_page_number, group_text, is_local=is_local, model=model)
...
def process_toc_no_page_numbers(toc_content, toc_page_list, page_list, is_local, start_index=1, model=None, logger=None):
    page_contents=[]
    token_lengths=[]
    toc_content = toc_transformer(toc_content, is_local=is_local, model=model)
    ...
    for group_text in group_texts:
        toc_with_page_number = add_page_number_to_toc(group_text, toc_with_page_number, is_local=is_local, model=model)
...
def process_toc_with_page_numbers(toc_content, toc_page_list, page_list, is_local, toc_check_page_num=None, model=None, logger=None):
    toc_with_page_number = toc_transformer(toc_content, is_local=is_local, model=model)
    ...
    toc_with_physical_index = toc_index_extractor(toc_no_page_number, main_content, is_local=is_local, model=model)
    ...
    toc_with_page_number = process_none_page_numbers(toc_with_page_number, page_list, model=model, is_local=is_local)
    logger.info(f'toc_with_page_number: {toc_with_page_number}')
...
def process_none_page_numbers(toc_items, page_list, is_local, start_index=1, model=None):
    for i, item in enumerate(toc_items):
        if "physical_index" not in item:
            ...
            del item_copy['page']
            result = add_page_number_to_toc(page_contents, item_copy, is_local=is_local, model=model)
...
def check_toc(page_list, is_local, opt=None, logger=None):
    toc_page_list = find_toc_pages(start_page_index=0, page_list=page_list, opt=opt, is_local=is_local, logger=logger)
    if len(toc_page_list) == 0:
        print('no toc found')
        return {'toc_content': None, 'toc_page_list': [], 'page_index_given_in_toc': 'no'}
    else:
        print('toc found')
        toc_json = toc_extractor(page_list, toc_page_list, opt.model, is_local=is_local)
        ...
        if toc_json['page_index_given_in_toc'] == 'yes':
            ...
            while (toc_json['page_index_given_in_toc'] == 'no' and
                ...
                additional_toc_json = toc_extractor(page_list, additional_toc_pages, opt.model, is_local=is_local)
...
async def fix_incorrect_toc(toc_with_page_number, page_list, incorrect_results, is_local, start_index=1, model=None, logger=None):
    ...
    async def process_and_check_item(incorrect_item):
        ...
        physical_index_int = single_toc_item_index_fixer(incorrect_item['title'], content_range, model=model, is_local=is_local)

        # Check if the result is correct
        check_item = incorrect_item.copy()
        check_item['physical_index'] = physical_index_int
        check_result = await check_title_appearance(check_item, page_list, is_local=is_local, start_index=start_index, model=model)
...
async def fix_incorrect_toc_with_retries(toc_with_page_number, page_list, incorrect_results, is_local, start_index=1, max_attempts=3, model=None, logger=None):
    ...
    while current_incorrect:
        ...
        current_toc, current_incorrect = await fix_incorrect_toc(current_toc, page_list, current_incorrect, is_local, start_index, model, logger)
        fix_attempt += 1
...
################### verify toc #########################################################
async def verify_toc(page_list, list_result, is_local, start_index=1, N=None, model=None):
    ...
    tasks = [
        check_title_appearance(item, page_list, is_local=is_local, start_index=start_index, model=model)
...
################### main process #########################################################
async def meta_processor(page_list, is_local, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    print(mode)
    print(f'start_index: {start_index}')

    if mode == 'process_toc_with_page_numbers':
        toc_with_page_number = process_toc_with_page_numbers(toc_content, toc_page_list, page_list, is_local, toc_check_page_num=opt.toc_check_page_num, model=opt.model, logger=logger)
    elif mode == 'process_toc_no_page_numbers':
        toc_with_page_number = process_toc_no_page_numbers(toc_content, toc_page_list, page_list, is_local, model=opt.model, logger=logger)
    else:
        toc_with_page_number = process_no_toc(page_list, is_local, start_index=start_index, model=opt.model, logger=logger)
    ...
    accuracy, incorrect_results = await verify_toc(page_list, toc_with_page_number, is_local=is_local, start_index=start_index, model=opt.model)
    ...
    if accuracy > 0.6 and len(incorrect_results) > 0:
        toc_with_page_number, incorrect_results = await fix_incorrect_toc_with_retries(toc_with_page_number, page_list, incorrect_results, is_local, start_index=start_index, max_attempts=3, model=opt.model, logger=logger)
        return toc_with_page_number
    else:
        if mode == 'process_toc_with_page_numbers':
            return await meta_processor(page_list, is_local, mode='process_toc_no_page_numbers', toc_content=toc_content, toc_page_list=toc_page_list, start_index=start_index, opt=opt, logger=logger)
        elif mode == 'process_toc_no_page_numbers':
            return await meta_processor(page_list, is_local, mode='process_no_toc', start_index=start_index, opt=opt, logger=logger)
        else:
            raise Exception('Processing failed')
        
 
async def process_large_node_recursively(node, page_list, is_local, opt=None, logger=None):
    ...
    if node['end_index'] - node['start_index'] > opt.max_page_num_each_node and token_num >= opt.max_token_num_each_node:
        ...
        node_toc_tree = await meta_processor(node_page_list, is_local, mode='process_no_toc', start_index=node['start_index'], opt=opt, logger=logger)
        node_toc_tree = await check_title_appearance_in_start_concurrent(node_toc_tree, page_list, is_local=is_local, model=opt.model, logger=logger)
    ...
    if 'nodes' in node and node['nodes']:
        tasks = [
            process_large_node_recursively(child_node, page_list, is_local, opt, logger=logger)
...
async def tree_parser(page_list, opt, is_local, doc=None, logger=None):
    check_toc_result = check_toc(page_list, is_local, opt=opt, logger=logger)
    ...
    toc_with_page_number = await check_title_appearance_in_start_concurrent(toc_with_page_number, page_list, is_local=is_local, model=opt.model, logger=logger)
    ...
    tasks = [
        process_large_node_recursively(node, page_list, is_local, opt, logger=logger)
...
def page_index_main(doc, opt=None):
    ...
    async def page_index_builder():
        structure = await tree_parser(page_list, opt, opt.is_local, doc=doc, logger=logger)
        ...
        if opt.if_add_node_summary == 'yes':
            if opt.if_add_node_text == 'no':
                add_node_text(structure, page_list)
            await generate_summaries_for_structure(structure, is_local=opt.is_local, model=opt.model)
            if opt.if_add_node_text == 'no':
                remove_structure_text(structure)
            if opt.if_add_doc_description == 'yes':
                # Create a clean structure without unnecessary fields for description generation
                clean_structure = create_clean_structure_for_description(structure)
                doc_description = generate_doc_description(clean_structure, is_local=opt.is_local, model=opt.model)
                return {
